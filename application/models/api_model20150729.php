<?php
	...
	class Api_model extends CI_Model{
		...
		public function ser_goods_list($post_data){
			...
			//echo "<pre/>";    [pageCount] => 10      [startPage] => 0
			//print_r($post_data);
		...
		public function get_prepay_order(){
			...
			$query = $this->db->order_by('createTime','asc')->limit(200)->get('orderInfo');
			$data = $query->result_array();
			return $data;
		}
		...
		public function get_withdrawWXW_info($data){
			...
			if(empty($codeInfo)){
				...
				$status = $this->db->where('orderID',$codeInfo['orderID'])->update('orderInfo',array('withdrawAccount'=>$data['withdrawAccount'],'updateTime'=>time()));
				echo $this->db->last_query();
				if($status){
