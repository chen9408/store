	...
	*/	
	class Storeapi extends CI_Controller{				
		...
		/*
			$post_data=Array(
				'applicantName' => '王颖',
				'applicantCercCode' => '371322199504015442',
				'phoneNumber' => '13585545813',
				'insuredDays' => '10',
				'policyBeginDate' => '20150801',
				'insuredNames'    => '王颖',
				'insuredCercCodes'=> '371322199504015442'
			);
			print_r($post_data);
		*/
/*
     * @param policyBeginDate 起保日期
     * @param insuredDays     保险时长
     * @param applicantName   投保人姓名
     * @param phoneNumber     投保人手机号
     * @param applicantCercCode   投保人身份证号
     * @param insuredNames  以,分割    被保人姓名
     * @param insuredCercCodes  以,分割  被保人身份证号
*/
		...
		public function phoneFeeCallback(){
			...
			if($endChar=='C'){
				if($post_data['status']=="success"){
					$orderStatus = 6;
					$this->SMSRemind($post_data['out_trade_id'],'21125','');
				}else{
					$orderStatus = 4;
				}
				$data = array(
			...
			if($order_info["goodsSubType"] == 18){                            //话费
				...
				if(is_array($payment_status)){
					...
					if(is_array($redeemCode_array)){
						...
						$this->SMSRemind($post_data["orderID"],'21126','');
					}else{
		...
		*
		*/
		public function insuranceRefund(){
			$data = $this->api_model->get_insurance_refund();
			if(empty($data)){
				return ;
			}
			print_r($data);
			foreach ($data as $arr) {
				$payment_array = array( 
					"expenseAccountID"	=> $arr["accountID"],
					"incomeAccountID"	=> $this->config->item('mirrtalkSeller'),
					"MEPoints" 			=> $arr["MEPoints"],
					"WEPoints" 			=> 0,
					"withdrawAccount"	=> '',
					"changedType"		=> 20,
					"remark"			=> "购买保险失败退款-(".$arr['orderID'].")",
					"tradeNumber"		=> $arr["orderID"],
					"callbackURL"		=> base_url().'index.php/storeapi/refundCallback?goodsSubType='.$arr["goodsSubType"]
				);		
				$payment_status=$this->payment($payment_array);
				...
				if(is_array($payment_status)){
					$data = array(
						"orderID" 		=> $arr["orderID"],
						"status" 		=> 4,
						"adultInfo"		=> $arr['adultInfo'],
						"updateTime"	=> time()
					);
					$status = $this->api_model->update_insure_status($data);
					echo "qewqewqew";
				}
			}
		...
		*/
		public function exchangeRedpack(){
			...
			$param_array["sign"] = $this->sign->get_weixin_sign($param_array);
		...
		*/
		public function pinsOrderPayment(){
			...
			$payment_status = $this->payment($payment_array);
			if(!is_array($payment_status)){
				//$body = $this->sign->return_error_code("10103",'json','inside');
				$body = $this->sign->return_error_code($payment_status,'json','inside');
				$this->output
			    ->set_content_type('application/json')
			    ->set_output($body);
		...
		public function reward(){   
			...
			$reward_array = $this->sign->getReward_sign_array($reward_array,'keyValue');
			$url = $this->config->item('mirrtalk_api');
